...
enum {MAX_PUBLIC_COMMANDS=512, PUBLIC_PLAY_RING=32};
typedef struct {uint64_t address; int32_t tick,side,card_id,x,y; uint32_t packed;} PublicCommand;
typedef struct {uint64_t id; int32_t tick,side,card_id,x,y,form,known;} PublicPlay;
...
typedef struct {
  ...
  PublicCommand commands[MAX_PUBLIC_COMMANDS];
  int command_count, commands_valid;
} ChainFrame;

typedef struct {
  uint64_t battle,next_id;
  int32_t tick,elixir[2];
  int valid,command_count,play_count;
  PublicCommand commands[MAX_PUBLIC_COMMANDS];
  PublicPlay plays[PUBLIC_PLAY_RING];
} PublicHistory;
...
static void read_public_commands(int fd,uint64_t base,ChainFrame *frame) {
  uint64_t queue=0,data=0,player=0,context=0,avatars=0;
  int32_t count=-1,capacity=-1,accounts[2][2];
  if(!read_exact(fd,frame->battle+0x38,&queue,8)||!queue||
     !read_exact(fd,queue+8,&data,8)||!read_exact(fd,queue+0x10,&capacity,4)||
     !read_exact(fd,queue+0x14,&count,4)||count<0||count>MAX_PUBLIC_COMMANDS||capacity<count||capacity>4096||(count&&!data))return;
  if(!read_exact(fd,frame->player_state+0xe0,&player,8)||!player||
     !read_exact(fd,player+0x10,&context,8)||!context||!read_exact(fd,context+0x98,&avatars,8)||!avatars)return;
  for(int side=0;side<2;side++){
    uint64_t avatar=0;
    if(!read_exact(fd,avatars+0x30+side*8,&avatar,8)||!avatar||!read_exact(fd,avatar,accounts[side],8))return;
  }
  if(!memcmp(accounts[0],accounts[1],8))return;
  for(int i=0;i<count;i++){
    uint64_t ptr=0,vt=0,spell=0;uint8_t raw[0x50];int32_t account[2];
    if(!read_exact(fd,data+i*8,&ptr,8)||!ptr||!read_exact(fd,ptr,raw,sizeof(raw)))return;
    memcpy(&vt,raw,8);
    // Version-pinned DoSpellCommand vtable; other command kinds are not plays.
    if(vt!=base+0x190edd8)continue;
    PublicCommand c={.address=ptr,.side=-1,.card_id=-1};
    memcpy(&c.tick,raw+0xc,4);memcpy(account,raw+0x14,8);memcpy(&c.x,raw+0x28,4);memcpy(&c.y,raw+0x2c,4);
    memcpy(&spell,raw+0x38,8);memcpy(&c.packed,raw+0x48,4);
    if(!spell||!read_exact(fd,spell+0x40,&c.card_id,4))return;
    for(int side=0;side<2;side++)if(!memcmp(account,accounts[side],8))c.side=side;
    if(c.side<0||c.tick<0||c.tick>10000||c.x<0||c.x>=18000||c.y<0||c.y>=32000||
       c.card_id<25000000||c.card_id>=30000000)return;
    frame->commands[frame->command_count++]=c;
  }
  frame->commands_valid=1;
}

static void update_public_history(PublicHistory *h,const ChainFrame *f,const PlayerFrame players[2],int tick,int active,int coherent){
  if(!active){memset(h,0,sizeof(*h));return;}
  if(!coherent)return;
  if(h->battle!=f->battle||tick<h->tick){memset(h,0,sizeof(*h));h->battle=f->battle;}
  if(h->valid&&tick==h->tick)return;
  if(h->valid&&f->commands_valid&&tick>h->tick&&tick-h->tick<=4){
    for(int side=0;side<2;side++){
      const PublicCommand *candidate=NULL;int due=0;
      for(int i=0;i<h->command_count;i++){
        const PublicCommand *c=&h->commands[i];
        if(c->side==side&&c->tick>=h->tick&&c->tick<tick){candidate=c;due++;}
      }
      if(due!=1)continue;
      int still_queued=0;
      for(int i=0;i<f->command_count;i++)if(f->commands[i].address==candidate->address&&f->commands[i].tick==candidate->tick)still_queued=1;
      int cost=(int)(candidate->packed>>28),drop=h->elixir[side]-players[side].elixir_raw;
      // Adjacent-frame cost evidence, not a long-window net-elixir comparison.
      if(still_queued||cost<1||cost>10||drop<=0||drop>cost*10000||cost*10000-drop>(tick-h->tick)*1000)continue;
      if(h->play_count==PUBLIC_PLAY_RING){memmove(h->plays,h->plays+1,(PUBLIC_PLAY_RING-1)*sizeof(PublicPlay));h->play_count--;}
      int form=(int)(candidate->packed&3),choice=(int)((candidate->packed>>4)&7);
      h->plays[h->play_count++]=(PublicPlay){++h->next_id,candidate->tick,side,candidate->card_id,
          candidate->x,candidate->y,form,form<=2&&choice==0};
    }
  }
  h->tick=tick;h->valid=f->commands_valid;h->command_count=f->commands_valid?f->command_count:0;
  memcpy(h->commands,f->commands,(size_t)h->command_count*sizeof(PublicCommand));
  for(int side=0;side<2;side++)h->elixir[side]=players[side].elixir_raw;
}

static void emit_public_history(const PublicHistory *h){
  printf(",\"public_history_schema\":\"native_spend_confirmed_v1\",\"public_history_source_valid\":%s,\"public_plays\":[",h->valid?"true":"false");
  for(int i=0;i<h->play_count;i++){
    const PublicPlay *p=&h->plays[i];if(i)putchar(',');
    printf("{\"id\":%" PRIu64 ",\"tick\":%d,\"side\":%d,\"card_id\":%d,\"x\":%d,\"y\":%d,\"form_flags\":%d,\"known\":%s}",
           p->id,p->tick,p->side,p->card_id,p->x,p->y,p->form,p->known?"true":"false");
  }
  putchar(']');
}
...
static int read_frame(int fd, uint64_t libg, uint64_t manager_rva,
  ...
  *coherent = 1;
  ...
  if (unified) read_public_commands(fd,libg,frame);
...
int main(int argc, char **argv) {
  ...
  uint64_t sequence = 0;
  PlayerCache cache = {0};
  PublicHistory *history=calloc(1,sizeof(*history));
  ChainFrame *frame = calloc(1, sizeof(*frame));
  if (!frame||!history) { free(frame);free(history);close(fd);return 5; }
  PlayerFrame players[2];
  while ((!max_frames || sequence < max_frames) && (kill(pid, 0) == 0 || errno == EPERM)) {
    ...
    update_public_history(history,frame,players,tick,active,coherent);
    ...
    if(unified)emit_public_history(history);
    if (unified) {
      printf(",\"entities\":[");
